...
- Current public artifact: `tb-155.0a1-b86-wu-gok` (real installer; its release body names
  `417a7c73e7e7116b3e87201194001274a535381e`, its tag resolves to `9c3fdc…`, and its embedded
...
| GM-08 | Continuous color picker and bidirectional color-space translator | Packaged anchored editor mounts local continuous HSL controls, direct named/HEX/HEX8/RGB/A/HSL/A/HSV/HWB/CIELAB/LCH/OKLab/OKLCH/CMYK entry, translated copy rows, clipping/gamut status, and contrast readout; translator tests pass | Partial / open: Word-depth typography, eyedropper, presets/import-export, every-element coverage, and built-artifact capture remain open |
| GM-09 | Browser tabs with overflow, pinning, grouping, four searches, and bulk-close preview | Packaged preview now carries the design-defined core: persisted active/order/pins, stable pinned region, measured overflow, searchable all-tabs popover with its own regex builder, drag/keyboard/context movement, and tab appearance entry | Partial / open: grouping, per-group and group-name searches, cross-window master scope, safe containing/not-containing bulk-close, and built-artifact captures remain open |
