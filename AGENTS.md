...
|---|---|
| "Every task that changes a repository ends with the work merged into the default branch and pushed" | **RESOLVED 2026-07-29 on explicit user direction.** For most of this project's life `main` held only `b5330388a96` (the submodule add) and the restyle lived solely on `design-import/thunderbird-3pane`, which is why this row read "Unresolved" and why the standing rule was *do not merge on your own initiative — raise it and get direction*. The direction was given, and `origin/main` was fast-forwarded to `4f23a444d0e`. It was a **pure fast-forward**: `git rev-list --left-right --count origin/main...HEAD` was `0  55`, so no merge commit, no conflict and no history rewrite, and it was done as `git push origin HEAD:main` — a ref update, with **no checkout of this working tree** (see [§5.6](#56-github-pages-is-published-with-plumbing-never-a-checkout) for why that matters). The two branches are now identical. **Keep them that way**: a push to the working branch that is not also fast-forwarded to `main` re-opens this gap. Pushing to `origin` remains the only permitted remote — `upstream` never. |
